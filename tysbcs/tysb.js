...
function initParams() {
    // 从输入控件读取参数
    const hMin = parseInt(document.getElementById('t-h-min')?.value || T_PARAMS.hMin);
    const hMax = parseInt(document.getElementById('t-h-max')?.value || T_PARAMS.hMax);
    const sMin = parseInt(document.getElementById('t-s-min')?.value || T_PARAMS.sMin);
    const sMax = parseInt(document.getElementById('t-s-max')?.value || T_PARAMS.sMax);
    const vMin = parseInt(document.getElementById('t-v-min')?.value || T_PARAMS.vMin);
    const vMax = parseInt(document.getElementById('t-v-max')?.value || T_PARAMS.vMax);
    
    T_PARAMS = {
        hMin: isNaN(hMin) ? 30 : hMin,
        hMax: isNaN(hMax) ? 45 : hMax,
        sMin: isNaN(sMin) ? 75 : sMin,
        sMax: isNaN(sMax) ? 135 : sMax,
        vMin: isNaN(vMin) ? 50 : vMin,
        vMax: isNaN(vMax) ? 255 : vMax
    };
    
    // 读取F_PIXEL参数
    const fHMin = parseInt(document.getElementById('f-h-min')?.value || F_PIXEL_PARAMS.hMin);
    const fHMax = parseInt(document.getElementById('f-h-max')?.value || F_PIXEL_PARAMS.hMax);
    const fSMin = parseInt(document.getElementById('f-s-min')?.value || F_PIXEL_PARAMS.sMin);
    const fSMax = parseInt(document.getElementById('f-s-max')?.value || F_PIXEL_PARAMS.sMax);
    const fVMin = parseInt(document.getElementById('f-v-min')?.value || F_PIXEL_PARAMS.vMin);
    const fVMax = parseInt(document.getElementById('f-v-max')?.value || F_PIXEL_PARAMS.vMax);
    
    F_PIXEL_PARAMS = {
        hMin: isNaN(fHMin) ? 30 : fHMin,
        hMax: isNaN(fHMax) ? 65 : fHMax,
        sMin: isNaN(fSMin) ? 75 : fSMin,
        sMax: isNaN(fSMax) ? 255 : fSMax,
        vMin: isNaN(fVMin) ? 50 : fVMin,
        vMax: isNaN(fVMax) ? 255 : fVMax
    };
...
function countTFPixels(centerX, centerY) {
    ...
    for (let y = minY; y <= maxY; y++) {
        ...
        for (let x = minX; x <= maxX; x++) {
            ...
            // 判断金色像素（F）
            if (h >= F_PIXEL_PARAMS.hMin && h <= F_PIXEL_PARAMS.hMax && 
                s >= F_PIXEL_PARAMS.sMin && s <= F_PIXEL_PARAMS.sMax && 
                v >= F_PIXEL_PARAMS.vMin && v <= F_PIXEL_PARAMS.vMax) {
                fPixel++;
...
function fastRgbToHsv(r, g, b) {
    ...
    let h = 0, s = 0;
    const v = Math.round(max * 255); // 修改为固定值(0-255)
    ...
    if (delta > 0) {
        ...
        // 简化S计算
        s = Math.round((delta / max) * 255); // 修改为固定值(0-255)
    }
    return { h, s, v };
}
...
function judgeAndShowResult(tPixel, fPixel) {
    ...
    if (fPixel === 0) {
        ...
        const resultText = `${ratio > JUDGE_RATIO ? '识别结果：太阳之子' : '识别结果：非太阳之子'}\nT像素: ${tPixel}, F像素: ${fPixel}\nTF比例: ${ratioPercentage}% (判定阈值: ${(JUDGE_RATIO * 100).toFixed(2)}%)`;
        updateResult(
            resultText,
